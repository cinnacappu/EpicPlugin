...
public class CommandGod implements CommandExecutor {
    ...
    public CommandGod(EpicPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (player.hasPermission("coolplugin.godmode")) {
                if (this.god_list.contains(player)) {
                    this.god_list.remove(player);
                    player.removePotionEffect(PotionEffectType.DAMAGE_RESISTANCE);
                    player.sendMessage("§2§l[EP]§r§b God mode is now turned off!");
                    ...
                    player.sendMessage("§2§l[EP]§r§b God mode is now turned on!");
                }
            } else {
